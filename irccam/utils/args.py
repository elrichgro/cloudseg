...
def parse_args():
    ...
    parser.add_argument("--use_clear_sky", help="subtract clear sky from raw ir data", default=True, type=lambda x: (str(x).lower() == "true"))
    parser.add_argument("--random_mask", help="augment training data with random masks", default=True, type=lambda x: (str(x).lower() == "true"))
    parser.add_argument("--random_rotations", help="augment training data with random rotations", default=True, type=lambda x: (str(x).lower() == "true"))
    parser.add_argument("--cluster", help="training on a cluster", default=False, type=lambda x: (str(x).lower() == "true"))
    ...
    # Paths
    parser.add_argument("--log_dir", help="path for saving logs", default="data/training_logs", type=lambda s: os.path.abspath(s))
    parser.add_argument("--dataset_root", help="path to dataset", default="data/datasets/optimized_4", type=lambda s: os.path.abspath(s))

    # Logging
    parser.add_argument("--use_wandb", help="use weights and biases for logging", default=False, type=lambda x: (str(x).lower() == "true"))
